setClass(
    "finite_filters",
    slots = c(sfilter = "moving_average", lfilters = "list", rfilters = "list")
)
...
setMethod(
    "*",
    signature(e1 = "finite_filters", e2 = "moving_average"),
    function(e1, e2) {
        sym <- e1@sfilter * e2
...
setMethod(
    "*",
    signature(e1 = "moving_average", e2 = "finite_filters"),
    function(e1, e2) {
        ...
        new_mat <- (mat_e1[, seq_along(new_e2)] %*% mat_e2)[
            seq_len(1 + length(e2@lfilters) + length(e2@rfilters)),
        ]

        max_lags <- min(
            sapply(new_e1, lower_bound),
            sapply(new_e2, lower_bound)
        )
        ...
        lfilters <- lapply(seq_len(nrow(lfilters)), function(i) {
            moving_average(
                lfilters[i, ],
                lags = new_lb_sym + (nrow(lfilters) - i) + 1,
                leading_zero = FALSE,
                trailing_zero = TRUE
            ) #why -1 ?
        })
...
setMethod(
    "*",
    signature(e1 = "finite_filters", e2 = "numeric"),
    function(e1, e2) {
        if (length(e2) == 1) {
...
#' @rdname filters_operations
#' @export
setMethod("*", signature(e1 = "ANY", e2 = "finite_filters"), function(e1, e2) {
    filter(e1, e2)
})
#' @rdname filters_operations
#' @export
setMethod("*", signature(e1 = "finite_filters", e2 = "ANY"), function(e1, e2) {
    e2 * e1
})
#' @rdname filters_operations
#' @export
setMethod(
    "+",
    signature(e1 = "numeric", e2 = "finite_filters"),
    function(e1, e2) {
        e2 + e1
    }
)
...
setMethod(
    "+",
    signature(e1 = "finite_filters", e2 = "missing"),
    function(e1, e2) e1
)
#' @rdname filters_operations
#' @export
...
#' @export
as.matrix.finite_filters <- function(
    x,
    sfilter = TRUE,
    rfilters = TRUE,
    lfilters = FALSE,
    ...
    mat <- do.call(
        cbind,
        c(lfilters_s, sfilter_s, rfilters_s, list(zero_as_na = zero_as_na))
    )
...
setMethod(
    "*",
    signature(e1 = "finite_filters", e2 = "finite_filters"),
    function(e1, e2) {
        ...
        new_e1 <- c(
            ...
            rep(
                list(e1@sfilter),
                1 +
                    (new_ub - length(e1@rfilters)) +
                    (new_lb - length(e1@lfilters))
            ),
            e1@rfilters
        )
        new_e2 <- c(
            ...
            rep(
                list(e2@sfilter),
                1 +
                    (new_ub - length(e2@rfilters)) +
                    (new_lb - length(e2@lfilters))
            ),
            e2@rfilters
        )
...
setMethod(
    ...
    function(e1, e2) {
        ...
        e1_lfilters_f <- c(
            e1_lfilters,
            rep(list(0), max(length(e2_lfilters) - length(e1_lfilters), 0))
        )
        e2_lfilters_f <- c(
            e2_lfilters,
            rep(list(0), max(length(e1_lfilters) - length(e2_lfilters), 0))
        )
        e1_rfilters_f <- c(
            e1_rfilters,
            rep(list(0), max(length(e2_rfilters) - length(e1_rfilters), 0))
        )
        e2_rfilters_f <- c(
            e2_rfilters,
            rep(list(0), max(length(e1_rfilters) - length(e2_rfilters), 0))
        )
...
setMethod(
    "[",
    signature(x = "finite_filters", i = "missing", j = "ANY"),
...
setMethod(
    "[",
    signature(x = "finite_filters", i = "ANY", j = "ANY"),
    function(x, i, j, ..., drop = TRUE) {
